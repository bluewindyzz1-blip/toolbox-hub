...
const baseUrl = process.env.PREVIEW_BASE_URL || "http://localhost:4173";
const previewHost = new URL(baseUrl).host;
const pages = [
  { slug: "calculator-hub", url: `${baseUrl}/calculator` },
  { slug: "monthly-take-home", url: `${baseUrl}/calculator/salary/monthly-take-home` },
  { slug: "loan-interest", url: `${baseUrl}/calculator/finance/loan-interest` },
  { slug: "brokerage-fee", url: `${baseUrl}/calculator/real-estate/brokerage-fee` },
];

const targets = await (await fetch("http://127.0.0.1:9222/json/list")).json();
const target = targets.find((item) => item.type === "page" && item.url.includes(previewHost));
if (!target?.webSocketDebuggerUrl) throw new Error("로컬 Preview 브라우저 디버그 연결을 찾지 못했습니다.");
...
for (const page of pages) {
  await cdp("Page.navigate", { url: page.url });
  await delay(900);
  ...
  await writeFile(`/home/ubuntu/workspace/toolbox-hub-github-preview/reports/mobile-p0-${page.slug}.png`, Buffer.from(image.data, "base64"));
  report.push({ page: page.url, ...metrics, screenshot: `mobile-p0-${page.slug}.png` });
}
