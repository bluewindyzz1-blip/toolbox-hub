...
const decisionMeta: Record<string, DecisionMeta> = {
  ...
  "deposit-interest": { interpretation: "예치기간과 금리를 기준으로 계산한 세전·세후 예상 이자입니다. 금융기관의 상품 조건과 이자 지급방식을 확인하세요.", reviewDate: "2026-01-01", sourceLabel: "금융위원회", sourceUrl: "https://www.fsc.go.kr", nextAction: "예금자보호와 상품별 우대조건을 확인하세요.", guidePath: "/guides/deposit-interest" },
  "savings": { interpretation: "정기적으로 납입하는 금액과 기간을 기준으로 계산한 만기 예상액입니다.", reviewDate: "2026-01-01", sourceLabel: "금융감독원 금융상품 한눈에", sourceUrl: "https://finlife.fss.or.kr", nextAction: "월 납입액과 만기일을 상품 설명서와 대조하세요.", guidePath: "/guides/savings" },
  "compound-interest": { interpretation: "이자가 원금에 다시 더해진다고 가정한 복리 시뮬레이션입니다. 실제 수익률은 세금·수수료·변동성에 따라 달라집니다.", reviewDate: "2026-01-01", sourceLabel: "금융위원회", sourceUrl: "https://www.fsc.go.kr", nextAction: "기간·수익률·수수료를 보수적으로 다시 계산하세요.", guidePath: "/guides/compound-interest" },
  "early-repayment-fee": { interpretation: "중도상환 원금과 수수료율을 이용한 예상 비용입니다. 금융기관의 면제 조건과 잔여기간 산식이 우선합니다.", reviewDate: "2026-01-01", sourceLabel: "금융감독원 금융소비자정보포털 파인", sourceUrl: "https://fine.fss.or.kr", nextAction: "갈아타기 전 중도상환수수료와 절감 이자를 함께 비교하세요.", guidePath: "/guides/early-repayment-fee" },
  "monthly-rent": { interpretation: "보증금의 기회비용을 월세에 더해 비교하는 단순 환산값입니다. 계약 조건과 법정 전월세전환율을 별도로 확인하세요.", reviewDate: "2026-01-01", sourceLabel: "국가법령정보센터", sourceUrl: "https://www.law.go.kr", nextAction: "계약서의 보증금·월세·특약을 확인하세요.", guidePath: "/guides/monthly-rent" },
  "jeonse-loan-interest": { interpretation: "전세대출 원금과 금리를 기준으로 한 예상 부담액이며 보증료와 변동금리는 포함되지 않을 수 있습니다.", reviewDate: "2026-01-01", sourceLabel: "주택도시보증공사", sourceUrl: "https://www.khug.or.kr", nextAction: "대출금리뿐 아니라 보증료와 보증 조건을 함께 확인하세요.", guidePath: "/guides/jeonse-loan-interest" },
  "mortgage": { interpretation: "주택담보대출의 원금·금리·기간에 따른 예상 상환액입니다. 실제 한도·담보평가·우대조건은 금융기관 심사 결과가 우선합니다.", reviewDate: "2026-01-01", sourceLabel: "금융감독원 금융상품 한눈에", sourceUrl: "https://finlife.fss.or.kr", nextAction: "취득 부대비용과 월 현금흐름까지 함께 계산하세요.", guidePath: "/guides/mortgage" },
  "vat": { interpretation: "일반적인 부가가치세율을 가정한 공급가액·세액·합계액 계산입니다. 과세유형과 거래별 적용 여부를 확인하세요.", reviewDate: "2026-01-01", sourceLabel: "국세청", sourceUrl: "https://www.nts.go.kr", nextAction: "사업자 유형과 신고기간을 국세청에서 확인하세요.", guidePath: "/guides/vat" },
  "acquisition-tax": { interpretation: "입력한 주택 가격과 조건을 기준으로 한 취득세 참고 계산입니다. 주택 수·지역·감면 여부에 따라 실제 세액이 달라질 수 있습니다.", reviewDate: "2026-01-01", sourceLabel: "위택스", sourceUrl: "https://www.wetax.go.kr", nextAction: "취득일 기준 주택 수와 감면 요건을 확인하세요.", guidePath: "/guides/acquisition-tax" },
  "property-tax": { interpretation: "공시가격과 세율 가정을 이용한 참고값입니다. 실제 고지세액은 공시가격·세부담상한·지방세 규정에 따라 달라집니다.", reviewDate: "2026-01-01", sourceLabel: "위택스", sourceUrl: "https://www.wetax.go.kr", nextAction: "공시가격과 지방세 고지서를 대조하세요.", guidePath: "/guides/property-tax" },
  "brokerage-fee": { interpretation: "거래금액과 거래유형을 기준으로 한 중개보수 상한 참고값입니다. 지역 조례와 계약 조건을 확인하세요.", reviewDate: "2026-01-01", sourceLabel: "국가법령정보센터", sourceUrl: "https://www.law.go.kr", nextAction: "중개대상물 확인설명서와 보수 협의 내용을 확인하세요.", guidePath: "/guides/brokerage-fee" },
  "pyeong": { interpretation: "제곱미터와 평 사이의 고정 환산값을 적용한 결과입니다.", reviewDate: "상시 기준", sourceLabel: "국가표준인증 통합정보시스템", sourceUrl: "https://standard.go.kr", nextAction: "공식 계약·공고 문서의 표시 단위를 우선 확인하세요.", guidePath: "/guides/pyeong" },
  "percentage": { interpretation: "입력한 기준값 대비 증감 또는 비율을 계산한 결과입니다.", reviewDate: "상시 기준", sourceLabel: "국가표준인증 통합정보시스템", sourceUrl: "https://standard.go.kr", nextAction: "비교 기준과 기간이 같은지 확인하세요.", guidePath: "/guides/percentage" },
  "pdf": { interpretation: "파일을 서버로 전송하지 않고 현재 브라우저에서 처리하는 PDF 작업입니다. 복잡한 폰트·표·암호화 문서는 결과가 달라질 수 있습니다.", reviewDate: "상시 기준", sourceLabel: "브라우저 내 처리 안내", nextAction: "변환 후 파일을 열어 페이지·서식·텍스트를 확인하세요.", guidePath: "/guides/pdf" },
  "pdf-merge": { interpretation: "선택한 PDF의 페이지를 브라우저에서 하나의 PDF로 결합합니다. 파일 순서와 페이지 수를 결과 파일에서 반드시 확인하세요.", reviewDate: "상시 기준", sourceLabel: "브라우저 내 처리 안내", nextAction: "병합 전 파일 순서를 정하고 병합 후 페이지를 열어 확인하세요.", guidePath: "/guides" },
  "pdf-excel": { interpretation: "텍스트 PDF에서 읽을 수 있는 내용을 CSV 형태로 추출하는 기능입니다. 표의 열·행과 서식은 원본과 달라질 수 있습니다.", reviewDate: "상시 기준", sourceLabel: "브라우저 내 처리 안내", nextAction: "CSV를 Excel에서 열어 열 구분과 숫자 형식을 확인하세요.", guidePath: "/guides" },
};
...
export function GuideDiscovery() {
  return <section className="guide-discovery container" aria-label="계산 가이드"><div><p className="eyebrow">PRACTICAL GUIDES</p><h2>계산 후 필요한 내용을<br />한 번에 확인하세요.</h2><p>계산 기준과 주의사항을 읽고, 다음에 사용할 관련 도구로 자연스럽게 이동할 수 있습니다.</p></div><Link href="/guides">전체 가이드 보기 <span>↗</span></Link></section>;
}
