...
export function SiteHeader() {
  ...
  const navItems = [{ href: "/", label: "홈" }, ...(data?.categories.filter((item) => item.parentId === null).map((item) => ({ href: getCategoryPath(item, data.categories), label: item.name })) ?? []), { href: "/search", label: "검색" }, { href: "/guides", label: "가이드" }, { href: "/about", label: "정보" }];
...
export function SiteFooter() {
  return (
    <footer className="site-footer">
      ...
      <div className="container footer-links"><Link href="/guide">이용방법</Link><Link href="/guides">계산기 가이드</Link><Link href="/faq">FAQ</Link><Link href="/privacy">개인정보처리방침</Link><Link href="/terms">이용약관</Link><Link href="/disclaimer">면책조항</Link><Link href="/cookie-policy">쿠키 및 광고 안내</Link><Link href="/contact">문의하기</Link><Link href="/search">도구 검색</Link></div>
    </footer>
  );
}
