...
const GUIDE_AUTHOR = "Carculate 운영팀";
const GUIDE_REVIEW_DATE = "2026년 9월 23일";

function RelatedCalculatorLinks({ tools, categories, top = false }: { tools: NonNullable<ReturnType<typeof useCatalog>["data"]>["tools"]; categories: NonNullable<ReturnType<typeof useCatalog>["data"]>["categories"]; top?: boolean }) {
  if (!tools.length) return null;
  return <section className={`guide-linked-tools${top ? " guide-top-tools" : ""}`} aria-label="관련 계산기">
    <p className="eyebrow">{top ? "USE THE CALCULATOR" : "CALCULATOR CONNECTION"}</p>
    <h2>{top ? "관련 계산기 바로 사용하기" : "관련 계산기"}</h2>
    {top && <p className="guide-tool-prompt">이 가이드의 핵심 항목을 바로 계산해 보고, 결과 아래에서 관련 가이드도 이어서 확인하세요.</p>}
    <div className="related-tools">{tools.map((tool) => <Link key={tool.id} href={getToolPath(tool, categories)}><span>{tool.kind.toUpperCase()}</span><strong>{tool.title}</strong><small>{tool.description}</small><b>계산기 열기 →</b></Link>)}</div>
  </section>;
}
...
export function GuidePage({ slug }: { slug: string }) {
  ...
  const relatedTools = guide.relatedToolSlugs.map((toolSlug) => catalog.tools.find((tool) => tool.slug === toolSlug)).filter((tool): tool is NonNullable<typeof tool> => Boolean(tool));
