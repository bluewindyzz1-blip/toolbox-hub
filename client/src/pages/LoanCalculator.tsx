...
export default function LoanCalculator() {
  const [initial] = useState(() => readSharedLoanValues({ amount: 30000, rate: 4.2, years: 3, method: "annuity" }));
  const initialMethod: LoanMethod = initial.method === "principal" ? "principal" : initial.method === "bullet" ? "bullet" : "annuity";
  const [amount, setAmount] = useState(initial.amount);
  const [rate, setRate] = useState(initial.rate);
  const [years, setYears] = useState(initial.years);
  const [method, setMethod] = useState<LoanMethod>(initialMethod);
  const [applied, setApplied] = useState({ amount: initial.amount, rate: initial.rate, years: initial.years, method: initialMethod });
  ...
  return (
    <ToolMetaResolver slug="loan-interest">{(tool) => <ToolFrame index="04" tag="LOAN CALCULATOR" title="대출 이자 계산기" description="원리금균등·원금균등 상환 방식을 비교하고 월별 원금과 이자를 확인하세요.">
      ...
      <LoanRateComparison amount={applied.amount} years={applied.years} method={applied.method} activeRate={applied.rate} />
      <LoanShareAction values={applied} />
