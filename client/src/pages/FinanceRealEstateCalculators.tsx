...
import { CatalogBreadcrumb, CalculatorActions, LoanRateComparison, LoanShareAction, SeoHead, ToolKnowledge, ToolMetaResolver } from "@/components/CatalogSupport";
import { CommaNumberInput } from "@/components/CommaNumberInput";
import { ToolFrame } from "@/components/ToolLayout";
import { readSharedLoanValues } from "@/lib/calculatorHub";
import {
...
const specs: Record<CalculatorKind, ToolSpec> = {
  ...
  "brokerage-fee": {
    index: "22", tag: "REAL ESTATE / COST", title: "부동산 중개수수료 계산기", description: "주택 매매·임대차 거래금액을 기준으로 중개보수 상한액을 계산합니다.",
    method: "거래 유형과 금액 구간에 따른 상한요율을 적용하고, 해당 구간에 한도액이 있으면 한도액을 넘지 않도록 계산합니다.", example: "6억원 주택 매매는 0.4% 상한요율을 적용해 중개보수 상한액 240만원이 계산됩니다.", caution: "서울시 주택 기준의 상한요율이며, 실제 보수는 지역 조례와 개업공인중개사와의 협의, 부가가치세 여부에 따라 달라질 수 있습니다.", useCase: "주택 매매나 전·월세 계약의 초기 부대비용을 추정할 때 사용합니다.", interpretation: "표시 금액은 상한액이며 실제 지급액과 부가가치세는 계약 전에 확인해야 합니다.",
  },
  "acquisition-tax": {
...
function LoanTool({ kind }: { kind: "mortgage" | "equal-principal" | "bullet-loan" }) {
  ...
  const [initial] = useState(() => readSharedLoanValues({ amount: 30000, rate: 4, years: 30, method: kind === "mortgage" ? "annuity" : kind === "equal-principal" ? "principal" : "bullet" }));
  const [amount, setAmount] = useState(initial.amount);
  const [rate, setRate] = useState(initial.rate);
  const [years, setYears] = useState(initial.years);
  const [applied, setApplied] = useState({ amount: initial.amount, rate: initial.rate, years: initial.years });
