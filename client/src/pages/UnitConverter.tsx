...
const categoryLabels: Record<string, string> = { length: "길이", area: "면적", weight: "무게", volume: "부피", temperature: "온도", speed: "속도", data: "데이터" };

type UnitConverterProps = { initialCategory?: string; [key: string]: unknown };

export default function UnitConverter({ initialCategory = "length" }: UnitConverterProps = {}) {
  const startingCategory = unitDefinitions[initialCategory] ? initialCategory : "length";
  const startingUnits = Object.keys(unitDefinitions[startingCategory]);
  const [category, setCategory] = useState(startingCategory);
  const [from, setFrom] = useState(startingUnits[0]);
  const [to, setTo] = useState(startingUnits[1] ?? startingUnits[0]);
  ...
  return (
    <ToolFrame index="05" tag="UNIT ENGINE" title={`${categoryLabels[category]} 단위 변환기`} description={`자주 사용하는 ${Object.values(categoryLabels).join(", ")} 단위를 양방향으로 빠르게 변환합니다.`}>
