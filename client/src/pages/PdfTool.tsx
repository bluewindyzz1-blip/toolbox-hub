...
export default function PdfTool({ initialOperation = "word" }: { initialOperation?: Operation }) {
  ...
  return <ToolFrame index="01" tag="DOCUMENT ENGINE" title={operationLabels[operation]} description="선택한 파일은 현재 기기 안에서만 처리됩니다. 서버 업로드 없이 작업이 끝난 후 바로 다운로드하세요.">
    <section className="tool-workbench">
      <div className="work-grid conversion-primary"><div className="conversion-panel">
        <div className="conversion-lead"><p className="eyebrow">STEP 01 / FILE</p><h2>먼저 파일을 넣으세요.</h2><p>{operationLabels[operation]}에 사용할 {isImageToPdf ? "이미지" : "PDF"} 파일을 선택하면 됩니다.</p></div>
